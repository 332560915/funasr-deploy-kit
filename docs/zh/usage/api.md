...
当前同步识别接口适合短音频、功能验证和 Swagger 联调。
...
可以在 Swagger 页面中选择 `/api/v1/asr` 接口，上传文件并直接测试识别结果。Swagger 更适合使用短音频或小文件测试。
...
上传大小由 `MAX_UPLOAD_SIZE` 控制，默认样例为 `31457280`，即 30MB。HTTP API 会分块写入临时文件，超过限制时返回 `413`。

`POST /api/v1/asr` 是同步接口，请求会等待上传、转发 FunASR、识别完成和结果返回。
